...
CREATE TABLE IF NOT EXISTS public.aria_texts (
    ...
    translation text,
    verse_structure jsonb, -- {verses: [{text: "", translation: ""}]}
    created_at timestamp with time zone DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.sheet_music (
    ...
    publisher text,
    edition text,
    arrangement_type text, -- 'vocal_score', 'piano_reduction', 'orchestral', etc.
...
CREATE TABLE IF NOT EXISTS public.opera_recordings (
    ...
    platform text, -- 'youtube', 'spotify', 'apple_music', 'internal', etc.
    external_url text,
    file_path text, -- For internal hosting
    recording_type text, -- 'audio', 'video', 'live_performance'
    quality text, -- 'studio', 'live', 'historical'
...
CREATE TABLE IF NOT EXISTS public.opera_productions (
    ...
    cast jsonb, -- {roles: [{role_name: "", performer: ""}]}
...
CREATE TABLE IF NOT EXISTS public.opera_analytics (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    entity_type text NOT NULL, -- 'work', 'aria', 'recording', 'sheet_music'
    entity_id uuid NOT NULL,
    action_type text NOT NULL, -- 'view', 'download', 'play', 'favorite'
...
INSERT INTO public.arias (work_id, title, act_number, scene_number, duration_minutes, key_signature, aria_type, first_line)
SELECT 
    lw.id,
    'Nessun dorma',
    3,
...
FROM public.lyrical_works lw 
WHERE lw.title ILIKE '%turandot%' 
LIMIT 1;