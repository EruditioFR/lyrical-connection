...
const ArtistsList = () => {
  ...
  const filteredArtists = artists.filter(artist => {
    ...
    const searchMatch = !searchTerm || artist.stage_name.toLowerCase().includes(searchTerm.toLowerCase()) || artist.voice_type && artist.voice_type.toLowerCase().includes(searchTerm.toLowerCase()) || artist.location && artist.location.toLowerCase().includes(searchTerm.toLowerCase()) || artist.bio && artist.bio.toLowerCase().includes(searchTerm.toLowerCase());

    // Filtrer par localisation
    const locationMatch = !location || artist.location && artist.location.toLowerCase().includes(location.toLowerCase());
  ...
  return <Layout>
      ...
      <section className="bg-gradient-to-b from-muted to-background py-12 md:py-[30px]">
        <div className="container mx-auto px-4">
          ...
          <SearchFilters searchTerm={searchTerm} onSearchChange={setSearchTerm} voiceType={voiceType} onVoiceTypeChange={setVoiceType} location={location} onLocationChange={setLocation} repertoireFilters={repertoireFilters} onRepertoireFiltersChange={setRepertoireFilters} />
        </div>
      </section>
      ...
      {user ? <div className="container mx-auto px-4 md:px-6 py-8">
          <Tabs defaultValue="artists" className="w-full">
            ...
            <TabsContent value="generate-images" className="mt-6">
              <ImageGenerationPanel artists={artists} onImagesGenerated={handleImagesGenerated} />
            </TabsContent>
          </Tabs>
        </div> : <ArtistsGrid artists={artists} filteredArtists={filteredArtists} isLoading={isLoading} searchQuery={searchTerm} selectedVoiceTypes={voiceType ? [voiceType] : []} onResetFilters={resetFilters} />}
    </Layout>;
};
